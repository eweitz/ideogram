...
Ideogram.prototype.getBands=function(c,f,d){var a={},b,k,h,g,e,l;"chrBands"===c.slice(0,8)&&(b="native");"undefined"===typeof chrBands&&"native"!==b?(delimiter=/\t/,c=c.split(/\r\n|\n/),g=1):(delimiter=/ /,c="native"===b?eval(c):c,g=0);e=c[0].split(delimiter)[0];b="#chromosome"==e?"ncbi":"#chrom"==e?"ucsc":"native";e=c.length;if("ncbi"===b||"native"===b)for(;g<e-1;g++){if(b=c[g].split(delimiter),h=b[0],"undefined"===typeof d||-1!==d.indexOf(h))!1===h in a&&(a[h]=[]),k=b[7],b[8]&&(k+=b[8]),b={chr:h,
...
d.stopPx).attr("y2",a.stopPx);c=(new Date).getTime();ideo.debug&&console.log("Time in drawSyntenicRegions: "+(c-f)+" ms")};
...
"</defs>")};
Ideogram.prototype.getTaxids=function(){var c,f,d,a,b,k,h,g;k="taxid"in this.config;this.config.multiorganism="org"in this.config&&this.config.org instanceof Array||k&&this.config.taxid instanceof Array;g=this.config.multiorganism;if("org"in this.config){a=g?this.config.org:[this.config.org];f=[];h={};for(b=0;b<a.length;b++)for(c in d=a[b],this.organisms)this.organisms[c].commonName.toLowerCase()===d&&(f.push(c),g&&(h[c]=this.config.chromosomes[d]));this.config.taxids=f;g&&(this.config.chromosomes=h)}g?
(this.coordinateSystem="bp",k&&(f=this.config.taxid)):(k&&(f=[this.config.taxid]),this.config.taxids=f);return f};
