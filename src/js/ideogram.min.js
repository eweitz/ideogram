var Ideogram=function(c){this.config=c;this.config.container||(this.config.container="body");this.config.resolution||(this.config.resolution=850);this.config.rows||(this.config.rows=1);!1==="chrHeight"in c&&(c.chrHeight=500);this.bump=Math.round(c.chrHeight/125);c.showBandLabels&&(this.config.chrMargin+=20);this.config.annotationsPath?(this.config.annotationHeight||(this.config.annotationHeight=3),this.config.numAnnotTracks=this.config.annotationTracks?this.config.annotationTracks.length:1,this.config.annotTracksHeight=
this.config.annotationHeight*this.config.numAnnotTracks,"undefined"===typeof this.config.barWidth&&(this.config.barWidth=10)):this.config.annotTracksHeight=0;this.config.chrMargin=this.config.chrMargin+this.config.chrWidth+2*this.config.annotTracksHeight;c.onLoad&&(this.onLoadCallback=c.onLoad);this.coordinateSystem="iscn";this.maxLength={bp:0,iscn:0};this.organisms={9606:{commonName:"Human",scientificName:"Homo sapiens",scientificNameAbbr:"H. sapiens"},10090:{commonName:"Mouse",scientificName:"Mus musculus",
scientificNameAbbr:"M. musculus"}};this.config.annotationsPath&&!this.config.annotationHeight&&(this.config.annotationHeight=3);this.chromosomesArray=[];this.bandsToShow=[];this.chromosomes={};this.bandData={};this.init()};
...
" path")[0].getBBox(),f=document.querySelectorAll("#"+b.chr.id+" path")[0].getBBox(),d=d.y-30,f=f.y-29,m=a.chr.id+"_"+a.start+"_"+a.stop+"___"+b.chr.id+"_"+b.start+"_"+b.stop,syntenicRegion=h.append("g").attr("class","syntenicRegion").attr("id",m).on("click",function(){var a=this,b=d3.selectAll(".syntenicRegion").filter(function(b,c){return this!==a});b.classed("hidden",!b.classed("hidden"))}).on("mouseover",function(){var a=this;d3.selectAll(".syntenicRegion").filter(function(b,c){return this!==
...
a.stopPx).attr("y2",b.stopPx);c=(new Date).getTime();console.log("Time in drawSyntenicRegions: "+(c-e)+" ms")};
...
f.chromosomesArray[n].id),n);!0===f.config.showChromosomeLabels&&f.drawChromosomeLabels(f.chromosomes);1<f.config.rows&&f.putChromosomesInRows();n=(new Date).getTime();console.log("Time in drawChromosome: "+(n-q)+" ms");n=(new Date).getTime();console.log("Time constructing ideogram: "+(n-h)+" ms");if(f.onLoadCallback)f.onLoadCallback()}var e=!0===this.config.multiorganism,a,b,d,f=this,h=(new Date).getTime();if(0==e)"undefined"==typeof this.config.taxid&&(this.config.taxid="9606"),a=this.config.taxid,
b=[a],this.config.taxids=b,d=this.config.chromosomes.slice(),this.config.chromosomes={},this.config.chromosomes[a]=d;else for(this.coordinateSystem="bp",b=this.config.taxids,e=0;e<b.length;e++)a=b[e];this.config.annotationsPath&&d3.json(f.config.annotationsPath,function(a){f.rawAnnots=a.annots});e="";this.config.showChromosomeLabels&&(e="horizontal"==this.config.orientation?e+"labeledLeft ":e+"labeled ");this.config.annotationsLayout&&"overlay"===this.config.annotationsLayout&&(e+="faint");var g=
this.config.chrHeight+40;1<this.config.rows&&(g=this.config.rows*(g-40));d3.select(this.config.container).append("svg").attr("id","ideogram").attr("class",e).attr("width","97%").attr("height",g);for(var k=[],l=0,g=this.config.resolution,e=0;e<b.length;e++)a=b[e],bandDataFileNames={9606:"ideogram_9606_GCF_000001305.14_"+g+"_V1",10090:"ideogram_10090_GCF_000000055.19_NA_V2"},"undefined"===typeof chrBands?d3.xhr("../data/bands/ncbi/"+bandDataFileNames[a]).on("beforesend",function(b){b.taxid=a}).get(function(a,
d){f.bandData[d.taxid]=d.response;l+=1;l==b.length&&c()}):(f.bandData["9606"]=chrBands,c())};
