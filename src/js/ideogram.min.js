var Ideogram=function(d){this.config=JSON.parse(JSON.stringify(d));this.debug=!1;this.config.bandDir||(this.config.bandDir="data/bands/");this.config.container||(this.config.container="body");this.config.resolution||(this.config.resolution=850);!1==="showChromosomeLabels"in this.config&&(this.config.showChromosomeLabels=!0);this.config.chrMargin||(this.config.chrMargin=10);if(!this.config.orientation){var f="vertical";this.config.orientation=f}if(!this.config.chrHeight){var b=this.config.container,
a=document.querySelector(b).getBoundingClientRect(),f="vertical"===f?a.height:a.width;"body"==b&&(f=500);this.config.chrHeight=f}this.config.chrWidth||(b=10,f=this.config.chrHeight,900>f&&500<f?b=Math.round(f/40):900<=f&&(b=Math.round(f/45)),this.config.chrWidth=b);this.config.showBandLabels||(this.config.showBandLabels=!1);this.config.brush||(this.config.brush=!1);this.config.rows||(this.config.rows=1);this.bump=Math.round(this.config.chrHeight/125);this.adjustedBump=!1;200>this.config.chrHeight&&
(this.adjustedBump=!0,this.bump=4);d.showBandLabels&&(this.config.chrMargin+=20);d.chromosome&&(this.config.chromosomes=[d.chromosome],!1==="showBandLabels"in d&&(this.config.showBandLabels=!0),!1==="rotatable"in d&&(this.config.rotatable=!1));this.initAnnotSettings();this.config.chrMargin=this.config.chrMargin+this.config.chrWidth+2*this.config.annotTracksHeight;d.onLoad&&(this.onLoadCallback=d.onLoad);d.onDrawAnnots&&(this.onDrawAnnotsCallback=d.onDrawAnnots);d.onBrushMove&&(this.onBrushMoveCallback=
...
e.putChromosomesInRows();!0===e.config.brush&&e.createBrush();e.config.annotations&&e.drawAnnots(e.config.annotations);if(e.config.armColors){var t=e.config.armColors;e.colorArms(t[0],t[1])}var y=(new Date).getTime();e.debug&&console.log("Time in drawChromosome: "+(y-a)+" ms");var z=(new Date).getTime();e.debug&&console.log("Time constructing ideogram: "+(z-m)+" ms");if(e.onLoadCallback)e.onLoadCallback();if("rotatable"in e.config&&!1===e.config.rotatable)d3.selectAll(".chromosome").style("cursor",
"default");else d3.selectAll(".chromosome").on("click",function(){ideogram.rotateAndToggleDisplay(this.id)})}catch(A){console.log(A.stack)}}var a,c,h,k,g,l,e=this,m=(new Date).getTime(),n=[],p=0,r=this.config.resolution;h=e.getTaxids();e.config.taxids=h;for(k=0;k<h.length;k++)c=h[k],e.config.assembly||(e.config.assembly="default"),a=e.organisms[c].assemblies[e.config.assembly],a={9606:"native/ideogram_9606_"+a+"_"+r+"_V1.js",10090:"native/ideogram_10090_"+a+"_NA_V2.js",7227:"ucsc/drosophila_melanogaster_dm6.tsv"},
"undefined"===typeof chrBands?d3.xhr(e.config.bandDir+a[c]).on("beforesend",function(a){a.taxid=c}).get(function(a,b){e.bandData[b.taxid]=b.response;p+=1;p==h.length&&(f(),d())}):(e.bandData[c]=chrBands,f(),d())};
