...
e.putChromosomesInRows();!0===e.config.brush&&e.createBrush();e.config.annotations&&e.drawAnnots(e.config.annotations);if(e.config.armColors){var t=e.config.armColors;e.colorArms(t[0],t[1])}var y=(new Date).getTime();e.debug&&console.log("Time in drawChromosome: "+(y-a)+" ms");var z=(new Date).getTime();e.debug&&console.log("Time constructing ideogram: "+(z-m)+" ms");if(e.onLoadCallback)e.onLoadCallback()}catch(A){console.log(A.stack)}}var a,c,h,k,g,l,e=this,m=(new Date).getTime(),n=[],p=0,r=this.config.resolution;
h=e.getTaxids();e.config.taxids=h;for(k=0;k<h.length;k++)c=h[k],e.config.assembly||(e.config.assembly="default"),a=e.organisms[c].assemblies[e.config.assembly],a={9606:"native/ideogram_9606_"+a+"_"+r+"_V1.js",10090:"native/ideogram_10090_"+a+"_NA_V2.js",7227:"ucsc/drosophila_melanogaster_dm6.tsv"},"undefined"===typeof chrBands?d3.xhr(e.config.bandDir+a[c]).on("beforesend",function(a){a.taxid=c}).get(function(a,b){e.bandData[b.taxid]=b.response;p+=1;p==h.length&&(f(),d())}):(e.bandData[c]=chrBands,
f(),d())};
